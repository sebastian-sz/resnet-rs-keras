...
from common import inference_warmup, tpu_profiler_hook
...
from official.resnet import imagenet_input, lars_util, resnet_model
from official.resnet.configs import resnet_config
from tensorflow.core.protobuf import (  # pylint: disable=g-direct-tensorflow-import
    rewriter_config_pb2,
)
...
flags.DEFINE_integer(
    ...
    default=None,
    help=(
        "Depth of ResNet model to use. Must be one of {18, 34, 50, 101, 152,"
...
flags.DEFINE_string(
    ...
    default=None,
    help=(
        "A flag to override the data format used in the model. The value"
...
flags.DEFINE_bool(
    "transpose_input", default=None, help="Use TPU double transpose optimization"
)

flags.DEFINE_bool("use_cache", default=None, help=("Enable cache for training input."))
...
flags.DEFINE_float(
    ...
    default=None,
    help=(
        "keep_prob parameter of DropBlock. Will not be used if "
...
flags.DEFINE_integer(
    ...
    default=None,
    help=(
        "size parameter of DropBlock. Will not be used if dropblock_groups " "is empty."
    ),
)
...
flags.DEFINE_string(
    "mode", default="train_and_eval", help='One of {"train_and_eval", "train", "eval"}.'
)
...
flags.DEFINE_integer(
    ...
    default=None,
    help=(
        "Number of parallel threads in CPU for the input pipeline."
...
flags.DEFINE_integer(
    ...
    default=None,
    help=(
        "Number of TPU cores in total. For a single TPU device, this is 8"
...
flags.DEFINE_string(
    "bigtable_instance", None, "The Cloud Bigtable instance to load data from."
)
flags.DEFINE_string(
    "bigtable_table", "imagenet", "The Cloud Bigtable table to load data from."
)
flags.DEFINE_string(
    "bigtable_train_prefix", "train_", "The prefix identifying training rows."
)
flags.DEFINE_string(
    "bigtable_eval_prefix", "validation_", "The prefix identifying evaluation rows."
)
flags.DEFINE_string(
    "bigtable_column_family", "tfexample", "The column family storing TFExamples."
)
flags.DEFINE_string(
    "bigtable_column_qualifier", "example", "The column name storing TFExamples."
)
...
flags.DEFINE_bool(
    ...
    default=False,
    help=(
        'Whether to export additional metagraph with "serve, tpu" tags'
...
flags.DEFINE_float(
    "momentum", default=None, help=("Momentum parameter used in the MomentumOptimizer.")
)
...
flags.DEFINE_float("poly_rate", default=None, help=("Set LARS/Poly learning rate."))
...
flags.DEFINE_integer(
    ...
    64,
    "The number of steps at " "which the global step information is logged.",
)
...
flags.DEFINE_string(
    "model_name", "resnet", "Serving model name used for the model server."
)
...
def learning_rate_schedule(params, current_epoch):
    ...
    decay_rate = scaled_lr * lr_schedule[0][0] * current_epoch / lr_schedule[0][1]
    for mult, start_epoch in lr_schedule:
        decay_rate = tf.where(current_epoch < start_epoch, decay_rate, scaled_lr * mult)
    return decay_rate
...
def get_pretrained_variables_to_restore(checkpoint_path, load_moving_average=False):
    ...
    for v in tf.global_variables():
        ...
        tf.logging.info(
            "Init variable [%s] from [%s] in ckpt", v.op.name, variable_name_ckpt
        )
...
def resnet_model_fn(features, labels, mode, params):
    ...
    if params["dropblock_groups"]:
        ...
        dropblock_keep_prob = 1 - current_ratio * (1 - params["dropblock_keep_prob"])
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        if FLAGS.export_moving_average:
            ...
            tf.logging.info(
                "Restoring from the latest checkpoint: %s", restore_checkpoint
            )
...
def _select_tables_from_flags():
    ...
    train_prefix = _verify_non_empty_string(FLAGS.bigtable_train_prefix, "train_prefix")
    eval_prefix = _verify_non_empty_string(FLAGS.bigtable_eval_prefix, "eval_prefix")
    column_family = _verify_non_empty_string(
        FLAGS.bigtable_column_family, "column_family"
    )
...
def main(unused_argv):
    ...
    params = params_dict.override_params_dict(params, FLAGS.config_file, is_strict=True)
    ...
    resnet_classifier = tf.estimator.Estimator(
        model_fn=resnet_model_fn, config=config, params=params.as_dict()
    )

    assert params.precision == "bfloat16" or params.precision == "float32", (
        "Invalid value for precision parameter; " "must be bfloat16 or float32."
    )
    ...
    if FLAGS.mode == "eval":
        ...
        try:
            current_step = tf.train.load_variable(
                FLAGS.model_dir, tf.GraphKeys.GLOBAL_STEP
            )
        ...
        if FLAGS.mode == "train":
            ...
            if params.use_async_checkpointing:
                try:
                    from tensorflow.contrib.tpu.python.tpu import (  # pylint: disable=g-import-not-at-top
                        async_checkpoint,
                    )
